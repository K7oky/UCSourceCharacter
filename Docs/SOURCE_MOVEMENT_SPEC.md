...
#### Инвариант: корень пешки всегда axis-aligned

`UpdatedComponent` держится с **нулевым поворотом**. Ориентация игрока живёт исключительно в
`State.ViewAngles`; `PublishTransform` переутверждает `FQuat::Identity` каждый кадр.

Две причины, обе обязательные:

1. **Камера — дочерний компонент** и вращается относительно корня. Любой поворот, оставленный на
   корне, прибавится к углам вида второй раз. Авторский поворот читается в `State.ViewAngles` в
   `BeginPlay` и там же обнуляется — иначе пешка, поставленная под yaw `Y`, смотрит по `2Y`, а
   движется по `Y` (при `Y = 180` — полностью инвертированное управление).
2. **Солвер метает AABB.** `FCollisionShape::MakeBox` выровнен по мировым осям и поворот актора
   игнорирует, так что повёрнутый корень заставил бы `USourceHullComponent` рисовать не тот бокс,
   который реально метается. При pitch/roll он бы ещё и сместился: хал Source не центрирован
   относительно origin (`mins.z = 0`, `maxs.z = 72`).

Меш тела, которому нужно смотреть по направлению взгляда, вешается на **дочерний** компонент,
управляемый yaw'ом вида, а не на корень.

Контракт «yaw применяется ровно один раз» закреплён тестами `SourceMovement.View.*`.
...
### Реализованные на текущий момент (Phase 4 + 6)

`SourceMovement_UnitTests.cpp` — 22 кейса, `SourceMovement_DuckTests.cpp` — 10 кейсов.

```
Duck.OnGroundDoesNotMoveOrigin                      VEC_DUCK_HULL_MIN == VEC_HULL_MIN ⇒ дельта нулевая
Duck.HullSwitchesOnlyWhenFullyDucked                хулл выбирается по m_bDucked, промежуточных высот нет
Duck.SpeedCropScalesMaxSpeed                        Q54: CS-версия скейлит и m_flMaxSpeed
Duck.CrouchJumpRaisesOriginByHalfHeightDelta        Q29 + Q30: +9, не +18, и немедленно
Duck.UnduckInAirDropsOriginByHalfHeightDelta        зеркально: −9
Duck.CrouchJumpClearsMoreThanPlainJump              сквозной замер: 54.7 → ~66 по ступням
Duck.UnduckClearsDuckedFlagImmediately              Q28: m_bDucked падает в первый же тик вставания
Duck.BlockedByCeilingSnapsBackToFullyDucked         ветка else от CanUnduck
Duck.SpamPenaltyDisablesDuckKey                     Q31/Q32: −2.0 за каждый фронт и спад, пол 1.5
Duck.JumpWhileDuckingUsesOverwriteBranch            Q45 через реальный ввод, а не подстановку флага
Duck.EyeOffsetFollowsSimpleSpline                   закрытая форма SetDuckedEyeOffset на каждом тике
```
...
## 8.8 Footstep / movement audio

Портирован **только слой решения**. `CBasePlayer::UpdateStepSound` (baseplayer_shared.cpp:632)
живёт в симуляции, потому что его каденс — тиковое состояние, а не презентация:

* `m_flStepSoundTime` — миллисекунды, убывает на `1000 * frametime` **один раз за usercmd**.
  Если бы таймер жил в аудиокомпоненте и убывал по render-delta, частота шагов зависела бы от FPS.
* пороги `velwalk/velrun` сравниваются со скоростью самой симуляции;
* `bWalking` управляет и громкостью, и длиной следующего интервала.

| Источник | Что делает | Где у нас |
|---|---|---|
| `UpdateStepSound` (baseplayer_shared.cpp:632) | решает, звучит ли шаг, и с какой громкостью | `SourceMovementSim_Audio.cpp` |
| `GetStepSoundVelocities` (:981) | ducking → 60/80, иначе 90/220. **CS не переопределяет** | там же |
| `SetStepSoundTime` (:999) | `bWalking ? 400 : 300`, `* sv_footstep_sound_frequency`, `+100` если ducking | там же |
| `CheckJumpButton` (cs:713) | `velocity.Length() > 126` → форсированный шаг `fvol = 1.0` | `SourceMovementSim_Move.cpp` |
| `PlayClientJumpSound` (c_cs_player.cpp:5473) | `Default.WalkJump`, безусловно | `CheckJumpButton` |
| `CCSPlayer::OnLand` (cs_player_shared.cpp:2579) | `fVelocity > 270` → `Default.Land` + run-step поверхности | `OnJump/OnLand` |

Результат пишется в `FSourceMovementEvents` (отдельно от `FSourceMovementTelemetry`: телеметрия —
диагностика, на которую нельзя реагировать, а это настоящий выход). `USourceMovementComponent`
выдаёт его делегатом **раз на шаг симуляции**, не раз на кадр — при 128 тик и 60 FPS в кадр
попадает два шага, и per-frame опрос потерял бы первый.

Воспроизведение — `USourceMovementAudioComponent`. Отсоединение компонента не меняет движение.

**Поверхности.** `FSourceWorldQuery_UE::ResolveSurfacePropsId` возвращает `EPhysicalSurface`
физматериала, а не `GetUniqueID()`: UniqueID стабилен только внутри одного процесса, и таблицу
(трение, jumpFactor, наборы звуков) против него авторить невозможно. Геометрия без физматериала даёт
`SurfaceType_Default` → фолбэк, поведение как раньше.

**Не портировано (Phase 7, помечено в коде):** ветка лестницы (`GetLadderSurface`, `fvol 0.5`,
`STEPSOUNDTIME_ON_LADDER`) и обе водные ветки, включая пропуск 1-из-4 шагов по колено в воде.
`LadderMove`/`CheckWater` — заглушки, поэтому эти ветки сейчас недостижимы.

**UNKNOWN-2 (последствие).** Без `surfaceproperties.txt` switch по `psurface->game.material`
не имеет данных; каждая поверхность берёт числа `CHAR_TEX_CONCRETE` — ровно то, что делает
`default:` в том же switch. Значения вынесены в `sv_footstep_volume_walk/run` (помечены NON-SOURCE).
...
| **6** | ✅ `Duck` (CS continuous state machine), `CanUnduck`, `Finish(Un)Duck`, `SetDuckedEyeOffset`, `HandleDuckingSpeedCrop`, `FixPlayerCrouchStuck`, `UpdateDuckJumpEyeOffset`, `DuckUntilOnGround`, duck-spam budget, crouch-jump. ✅ `CheckFalling`, `OnJump/OnLand`, stamina (Phase 4).<br>⬜ Осталось: `CheckStuck` (54-элементная jitter-таблица), `CheckMovingGround`/baseVelocity, touch list | §8.2 duck-группа + edge-cases §6 брифа |
